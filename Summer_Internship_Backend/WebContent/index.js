...
//building the table header only
...
//for accessing the particular row 

	//var y = document.querySelectorAll('[name="name1"]:checked');
// we used this because in our element we have a value that is equal to the Field1 from the database
...
function editInvoice(){
	var newInvAmt = document.getElementById("modal-edit-amount").value;
	var newNotes = document.getElementById("edit-notes").value;
	
	var y = document.querySelectorAll('[name="name1"]:checked');
	var editId= y[0]['value'];
	$.post('/H2HBABBA2587/edit',{uniqueId : editId,
								inamt : newInvAmt,
								nns : newNotes}, function(){
...
function deleteInvoice(){
	...
	for(let x=0;x<y.length;x++)
	{
		$.post('/H2HBABBA2587/delete',{idList : arr[x]});
	}
	window.location.reload();
